...
export default function DCLandingPage() {
  ...
  return (
    <>
      <header className="w-full bg-[#F5F5ED] py-4 px-6 flex justify-between items-center border-b border-gray-200">
        <div className="text-2xl font-bold text-[#4A7C44]">Sabor&Magia</div>
        <Link href="/usuario-login">
          <UserCircle2 className="w-10 h-10 text-gray-600 hover:text-[#4A7C44] transition-colors cursor-pointer" />
        </Link>
      </header>
          ...
          >
            <CarouselContent>
              {destaques.length > 0 ? (
                ...
                <CarouselItem>
                  <div className="h-[250px] md:h-[400px] w-full bg-gray-200 animate-pulse rounded-[2rem]" />
                </CarouselItem>
              )}
            </CarouselContent>
